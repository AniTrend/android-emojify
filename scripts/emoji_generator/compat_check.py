...
class MultiCodeEntry(TypedDict):
  emoji: object
  codes: list[str]
...
def fetch_preset(path: Path | None) -> tuple[dict[str, object] | None, str]:
  ...
  try:
    with urlopen(PRESET_URL, timeout=15) as response:  # noqa: S310
...
def multi_code_stats(
  records: list[dict[str, object]], field: str
) -> tuple[int, int, list[MultiCodeEntry]]:
  entries: list[MultiCodeEntry] = []
  for record in records:
    emoji = record.get("emoji")
  ...
  examples = sorted(
    entries, key=lambda entry: (-len(entry["codes"]), str(entry["emoji"]))
  )[:10]
...
def main() -> int:
  ...
  parser.add_argument(
    "--legacy", type=Path, default=LEGACY_PATH, help="frozen 1.9.7 JSON"
  )
  parser.add_argument(
    "--current", type=Path, default=CURRENT_PATH, help="current generated JSON"
  )
  parser.add_argument(
    "--preset-json",
    type=Path,
  ...
  if not isinstance(legacy_payload, list) or not isinstance(current_payload, list):
    print(
      "Both input files must contain JSON arrays of emoji records.", file=sys.stderr
    )
    return 2
  ...
  old_occurrences = [
    code for record in legacy for code in string_list(record.get("aliases"))
  ]
  current_occurrences = [
    code for record in current for code in string_list(record.get("shortCodes"))
  ]
  ...
  legacy_by_emoji = {
    record["emoji"]: record for record in legacy if isinstance(record.get("emoji"), str)
  }
  ...
  old_collision_codes = {
    code: sorted(emojis)
    for code, emojis in old_alias_to_emoji.items()
    if len(emojis) > 1
  }
  current_collision_codes = {
    code: sorted(emojis)
    for code, emojis in current_code_to_emoji.items()
    if len(emojis) > 1
  }
  legacy_multi_count, legacy_max_codes, legacy_multi_examples = multi_code_stats(
    legacy, "aliases"
  )
  current_multi_count, current_max_codes, current_multi_examples = multi_code_stats(
    current, "shortCodes"
  )

  old_fitzpatrick = [
    record for record in legacy if record.get("supports_fitzpatrick") is True
  ]
  ...
  preset_by_hex = (
    {str(key).upper(): value for key, value in preset.items()} if preset else {}
  )

  def preset_first(emoji: str) -> str | None:
    return (
      string_list(preset_by_hex.get(unicode_hex_key(emoji)))[0:1][0]
      if string_list(preset_by_hex.get(unicode_hex_key(emoji)))
      else None
    )
  ...
  canonical = {
    emoji: string_list(record.get("aliases"))
    for emoji, record in legacy_by_emoji.items()
  }
  legacy_records = list(legacy_by_emoji.items())
  canonical_results: dict[str, CanonicalResult] = {}
  for rule in (
    "A legacy-first",
    "B current-first-deterministic",
    "C emojibase-preset-first",
  ):
    matches = 0
    available = 0
    mismatches: list[tuple[str, str | None, str]] = []
    unavailable = 0
    for emoji, _record in legacy_records:
  ...
  print(
    f"1.9.7 alias count: {len(old_occurrences)} occurrences, {len(old_codes)} distinct"
  )
  ...
  print(
    f"Exact aliases still represented: {len(old_codes & current_codes)} / {len(old_codes)} distinct"
  )
  ...
  print(
    f"Legacy pairs on emoji with no current shortcode: {len(orphaned_legacy_pairs)}"
  )
  print()
  ...
  for code, emojis in sorted(old_collision_codes.items())[:12]:
    print(
      f"  {code}: {' | '.join(f'{emoji} ({format_unicode(emoji)})' for emoji in emojis)}"
    )
  ...
  for entry in legacy_multi_examples:
    print(
      f"  {entry['emoji']} ({format_unicode(cast('str', entry['emoji']))}): {', '.join(entry['codes'])}"
    )
  ...
  policy = (
    "legacy OR current" if MERGE_FITZPATRICK_CAPABILITY_WITH_OR else "current-only"
  )
  ...
  for emoji in sorted(SOURCE_LEGACY_TRUE_CURRENT_FALSE_FITZPATRICK):
    ...
    forms = [
      f":{alias}|{suffix}:" for alias in aliases for suffix in FITZPATRICK_SUFFIXES
    ]
  ...
  print(
    f"Merged legacy true, current false exceptions: {len(legacy_true_current_false)}"
  )
  print(f"Fitzpatrick-capable legacy aliases: {len(old_fitzpatrick)} emoji records")
  print(
    f"Legacy Fitzpatrick alias pairs: {len(fitz_pairs)}; distinct aliases: {len(fitz_aliases)}"
  )
  for record in sorted(old_fitzpatrick, key=lambda item: str(item.get("emoji"))):
    ...
    print(
      f"  {emoji} ({format_unicode(cast('str', emoji))}): aliases={','.join(aliases)}; forms={','.join(forms)}"
    )
  print()
  ...
  for emoji in sample_emojis:
    ...
    old_first = (
      string_list(old.get("aliases"))[0]
      if old and string_list(old.get("aliases"))
      else None
    )
    ...
    rendered = [
      f":{value}{suffix}:" if value is not None else "Unknown" for value in values
    ]
    print(
  ...
  for code in lost_codes:
    ...
    print(
      f"  {code}: {' | '.join(f'{emoji} ({format_unicode(emoji)})' for emoji in mapped)}"
    )
  print()
  print("Appendix: every legacy alias mapping reassigned away from its legacy emoji")
  for code, emoji in sorted(reassigned_pairs):
    sorted_destinations = sorted(current_code_to_emoji[code])
    print(
      f"  {code}: legacy={emoji} ({format_unicode(emoji)}); current="
      f"{' | '.join(f'{item} ({format_unicode(item)})' for item in sorted_destinations)}"
    )
  print()
  print(
    "Appendix: every reassigned alias whose Unicode mapping is not variation-selector-only"
  )
