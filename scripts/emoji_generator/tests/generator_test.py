...
SAMPLE_SHORTCODE_JSON = {
  "1F600": [
    "grinning",
    "grinning_face",
  ],  # emoji with existing shortcodes - should merge
...
def mock_both_data():
  ...
  with patch(
    "emoji_generator.sources.requests.get", side_effect=side_effect
  ) as mock_get:
    yield mock_get


def test_fetch_emoji_data(mock_both_data):
...
def test_parse_emoji_data():
  emoji1 = Emoji(
    ...
    shortcodes=["grinning_face"],
    skins=[
      Emoji(emoji="😀", label="Grinning Face Light Skin Tone", tone=SkinTone.LIGHT)
    ],
